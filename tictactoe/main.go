...
/*
Aufgabe: schreibe eine Testfunktion für Funktion

	getPossibleMoves(b board) (moves [][][]string, moveIndex [][]int)

Aufgabe: schreibe eine schön formatierte Ausgabe für

	possibleMoves, _ := getPossibleMoves(*b)
	fmt.Println(possibleMoves)

Aufgabe: schreibe eine Testfunktion für minmax mit Sieg, Niederlage und Unentschieden
Aufgabe: ändere die Funktion checkwin, sodass diese zwei Werte zurück gibt (winner und isDraw)
Aufgabe: programmiere den fehlenden Teil minmax für PLAYER_MIN
*/
var cnt int
...
func getPossibleMoves(b board) (moves [][][]string, moveIndex [][]int) {
	player := b.turn
	for i := 0; i < SIZE; i++ {
		for j := 0; j < SIZE; j++ {
